...
function getInitialData(that, callback) {
    $.get(window.fmBaseDir + 'getTopLevelOrgs',{}).then(function(data) {
...
function getPortfolioChildren(that, obj, callback) {
    $.get(window.fmBaseDir + 'loadOrgChildren', {
        id: obj.id
    }).then(function(data) {
...
function editpersongoal() {
    ...
    window.location.href = window.fmGoalsDir + 'index?id='+ currentNodeId;
    removeSelectedBtnClassFromAll();
    $('.fm-right-form').hide();
